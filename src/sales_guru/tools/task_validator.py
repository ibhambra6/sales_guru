...
class TaskValidatorTool(BaseTool):
    ...
    def _extract_expected_fields(self, expected_output: str) -> List[str]:
        ...
        # Common field patterns for leads
        ...
        for line in lines:
            ...
            if is_list_item:
                ...
                if len(parts) > 1:
                    ...
                    if '"' in field_part:
                        # Extract from quotes
                        quoted = field_part.split('"')
                        if len(quoted) > 1:
                            fields.append(quoted[1].strip().lower().replace(' ', '_'))
                    # Look for a field name with a colon
                    elif ':' in field_part:
                        # Extract before the colon
                        field_name = field_part.split(':', 1)[0].strip().lower().replace(' ', '_')
                        fields.append(field_name)
                    else:
                        # Just use the whole part
                        fields.append(field_part.lower().replace(' ', '_'))
